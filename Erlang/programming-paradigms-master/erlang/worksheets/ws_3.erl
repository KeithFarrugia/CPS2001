...
calc_loop() ->
    receive
        {Pid, tag, stop} ->
            io:format("Calculator process stopped.~n"),
            Pid ! {tag,{ok, stopped}};
        
        {Pid, tag, {add, X, Y}} when is_number(X), is_number(Y) ->
            Sum = X + Y,
            io:format("Addition result: ~p~n", [Sum]),
            Pid ! {tag,{ok, Sum}},
            calc_loop();

        {Pid, tag, {sub, X, Y}} when is_number(X), is_number(Y) ->
            Difference = X - Y,
            io:format("Subtraction result: ~p~n", [Difference]),
            Pid ! {tag,{ok, Difference}},
            calc_loop();
...
%% -----------------------------------------------------------------------------
calc_link() ->
  Pid = spawn_link(fun calc_loop/0),
  Pid.
...
calc_super_loop(Restarts, Pid, CallerPid) ->
    receive
        {'EXIT', Pid, Reason} ->
            ...
            io:fwrite(user, "Calculator process exited with reason: ~p. Restarting.~n", [Reason]),
            NewPid = calc_link(),
            CallerPid ! NewPid,
            calc_super_loop(NewRestarts, NewPid, CallerPid);
        {CallerPid, tag, status} ->
            % Received a 'status' request. Respond with the current restart count.
            CallerPid ! {tag,{ok, Restarts}},
            calc_super_loop(Restarts, Pid, CallerPid);

        Msg ->
            % Received an unexpected message. Log it and continue the loop.
            io:fwrite(user, "Calculator supervisor received unexpected message: ~p~n", [Msg]),
            CallerPid ! Msg,
            calc_super_loop(Restarts, Pid, CallerPid)
    end.
