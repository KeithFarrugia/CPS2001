...
counter_loop(Count) -> 
  receive
    ...
    {get, From}       -> From ! {ok, Count}, counter_loop(Count);
    {put, From, Amt}  -> From ! {ok, put, Count}, counter_loop((Count+Amt));
    {inc, From} -> From ! {ok, inc, Count}, counter_loop((Count+1));
    {dec, From} -> From ! {ok, dec, Count}, counter_loop((Count-1));
    Msg               -> io:format("Unknown message: ~p~n", [Msg]), counter_loop(Count-1)
  end.
