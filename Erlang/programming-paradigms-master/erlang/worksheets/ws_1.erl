...
tuple_to_list2(_, N, S) when N =< 0 ->
  S;
tuple_to_list2 (A, N, S)->
  tuple_to_list2(A, N-1, [element(N,A)] ++ S).
...
%% empty list.
%% -----------------------------------------------------------------------------
for(Start, Max, Func) when Start == Max ->
  [Func(Start)];

for(Start, Max, Func) when Start < Max  ->
  [Func(Start) | for(Start+1, Max, Func)].
