...
handle({subscribe, Msisdn}, Store) ->
  case util:fetch(Msisdn, 1, Store) of
    ...
    false   -> NewStore = Store++[{Msisdn, 0, []}],
               {{ok, subscribed}, NewStore};
    
    % Is already Subscribed
    _Found  -> {{error, already_subscribed}, Store}
  end;
...
handle({unsubscribe, Msisdn}, Store) ->
  case util:fetch(Msisdn, 1, Store) of
    % Cannot unsubscribe for it is not subscribed (already unsubscribed)
    false   -> {{error, not_subscribed}, Store};
    
    % Subscirbed
    _Found  -> NewStore = util:remove(Msisdn, 1, Store),
...
handle({attach, Pid, Msisdn}, Store) ->
  ...
  case util:fetch(Pid, 2, Store) of
    % Pid is not in use
        false ->  case util:fetch(Msisdn, 1, Store) of
                    % Is not Subscribed
                    false -> {{error, not_subscribed}, Store};

                    % Is Subscribed but not Attached
                    {Msisdn, 0, Pending}   -> NewEntry = {Msisdn, Pid, []},
                                              NewStore = util:store(Msisdn, 1, Store, NewEntry),
                                              link(Pid),
                                              {{ok, attached, Pending}, NewStore};

                    % Is Subscribed and already Attached
                    {Msisdn, _, _} -> {{error, already_attached}, Store}
                  end;

    % Pid is already in use
    _PidFound -> {{error, already_attached}, Store}
...
handle({detach, Pid}, Store) ->
  case util:fetch(Pid, 2, Store) of
    % Pid was not found
    false                    -> {{error, not_attached}, Store};

    % Pid is found
    {Msisdn, Pid, Pending}   -> NewStore = util:store(Pid, 2, Store, {Msisdn, 0, Pending}),
                                unlink(Pid),
...
handle({send_msg, Pid, ToMsisdn, Msg}, Store) ->
  case util:fetch(Pid, 2, Store) of
    % Pid is not attached hence cannot send message
    false -> {{error, not_attached}, Store};
    ...
    {From_Misisdn, Pid, _} -> 
            case util:fetch(ToMsisdn, 1, Store) of
               % Not Subscribed
               false                          ->  {{error, to_not_subscribed}, Store};

               % Found but not attached
               {ToMsisdn, 0, Pending}         ->  NewStore = util:store(ToMsisdn, 1, Store, {ToMsisdn, 0, Pending++[{From_Misisdn, Msg}]}),
                                                  {{ok, msg_queued}, NewStore};

               % Found and attached
               {ToMsisdn, Pid_to, _Pending }  ->  server_gen:rpc_async(Pid_to, Msg),
                                                  {{ok, msg_sent}, Store}
            end
  end.
