  ...
  receive
    ...
    {From, Tag, {subscribe, Msisdn}} ->
      case util:fetch(Msisdn, 1, Store) of
        ...
        false  -> From ! {Tag,{ok, subscribed}},
                  New = {Msisdn, 0, []},
                  loop([New | Store]);
 
        % Already Subscribed
        _Found -> From ! {Tag,{error, already_subscribed}},
                  loop(Store)
      end;
    ...
    {From, Tag, {unsubscribe, Msisdn}} ->
      case util:fetch(Msisdn, 1, Store) of
        ...
        false -> From ! {Tag,{error, not_subscribed}},
                 loop(Store);

        % Subscirbed
        _Found  -> From ! {Tag,{ok, unsubscribed}},
                   New_St = util:remove(Msisdn, 1, Store),
                   loop(New_St)
      end
  end.
