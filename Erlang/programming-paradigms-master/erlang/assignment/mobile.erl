...
handle(tog_flight_mode, State = {Msisdn, Status}) ->
  case Status of 
    % Flight mode off -> Flight mode on
    flight_mode_off -> case switch:detach() of
                          % Successfully detached
                          {ok, detached} -> {{ok, flight_mode_on}, {Msisdn, flight_mode_on}};
                          % Unsuccessfull detachment
                          {error, Reason}-> {{error, Reason}, State}
                        end;
    % Flight mode on -> Flight mode off
    flight_mode_on  -> case switch:attach(Msisdn) of
                          % Successfully attached
                          {ok, attached, Pending} -> print_msgs(Pending),{{ok, flight_mode_off}, {Msisdn, flight_mode_off}};
                          % Unsuccessfull attachment
                          {error, Reason}-> {{error, Reason}, State}
                        end
...
handle({send_msg, ToMsisdn, Msg}, State) ->
  {switch:send_msg(ToMsisdn, Msg), State}.
